...
instance {-# OVERLAPPABLE #-} (RearrangeDel k env' target' env'',
    ...
    RearrangeDel k env (x ': target') env'' where
        rDel l = let (prependX, l') = removeHListElem @k @x @env @env' l
...
class HListElem k (x :: k) (env :: [k]) (env' :: [k]) | x env -> env' where
    removeHListElem :: HList env -> (HList xs -> HList (x ': xs), HList env')
    getHListElem :: HList env -> (HList xs -> HList (x ': xs))
